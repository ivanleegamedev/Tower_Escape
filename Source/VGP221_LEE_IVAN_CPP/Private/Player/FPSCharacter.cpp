...
AFPSCharacter::AFPSCharacter()
{
	...
	if (!FPSCameraComponent) {
		...
		FPSCameraComponent->SetupAttachment(CastChecked<USceneComponent, UCapsuleComponent>(GetCapsuleComponent())); // Attach to the capsule
		FPSCameraComponent->SetRelativeLocation(FVector(0.0f, 0.0f, 50.0f + BaseEyeHeight)); // Position the camera
		FPSCameraComponent->bUsePawnControlRotation = true; // Rotate the camera with the controller
	}
...
void AFPSCharacter::MoveForward(float value)
{
	...
	// Use UE_LOG or AddOnScreenDebugMessage to debug the value
	// GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Green, FString::Printf(TEXT("Moveforward: %f"), value));
